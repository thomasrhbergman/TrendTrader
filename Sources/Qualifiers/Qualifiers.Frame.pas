...
begin
  ...
  if (Data^.DocType = ntQualifier) then
  begin
    if Data^.Qualifier.IsCondition then
    begin
      TargetCanvas.Brush.Color := clWebMintcream;
      TargetCanvas.FillRect(CellRect);
    end
    else if not Data^.Qualifier.Enabled then
    begin
      TargetCanvas.Brush.Color := clWebMintcream;
      TargetCanvas.FillRect(CellRect);
    end;
        ...
        case Column of
          ...
          COL_NAME:
            begin
              ...
              if not Data^.Qualifier.Enabled then
          ...
          COL_TYPE_CONDITION:
            CellText := Data^.Qualifier.TypeCondition.ToString;
          COL_STARTUP_DATE:
            case Data^.Qualifier.TypeCondition of
              tcRealtime:
                CellText := '';
              tcEveryDay:
                CellText := FormatDateTime('hh:nn:ss', Data^.Qualifier.StartupDate);
              tcSpecificDate:
                CellText := FormatDateTime('DD.MM.YYYY hh:nn:ss', Data^.Qualifier.StartupDate);
            end;
          COL_INSTRUMENT1:
            if (Data^.Qualifier.TypeCondition = tcRealtime) then
            begin
              if (Data^.Qualifier.Instrument1.SokidInfo.SecurityType = 'OPT') or
                 (Data^.Qualifier.Instrument1.SokidInfo.SecurityType = 'FUT') then
                CellText := Format(rsInstrument, [Data^.Qualifier.Instrument1.SokidInfo.LocalSymbol,
                                                  Data^.Qualifier.Instrument1.SokidInfo.ContractId])
              else
                CellText := Format(rsInstrument, [Data^.Qualifier.Instrument1.SokidInfo.Symbol,
                                                  Data^.Qualifier.Instrument1.SokidInfo.ContractId])
            end
            else
              CellText := '';
          COL_INSTRUMENT1_VALUES:
            if (Data^.Qualifier.TypeCondition = tcRealtime) then
            begin
              if (Data^.Qualifier.Instrument1.TickType2 = ttNotSet) then
                CellText := Format('%.2f', [Data^.Qualifier.Instrument1.PriceValue1])
              else
                CellText := Format(rsValues, [Data^.Qualifier.Instrument1.PriceValue1,
                                              Data^.Qualifier.Instrument1.PriceValue2])
            end
            else
              CellText := '';
          COL_INSTRUMENT1_TICKTYPES:
            ...
            begin
              if (Data^.Qualifier.Instrument1.TickType2 = ttNotSet) then
                CellText := Data^.Qualifier.Instrument1.TickType1.ToString
              else
                CellText := Format(rsTickTypes, [Data^.Qualifier.Instrument1.TickType1.ToString,
                                                 Data^.Qualifier.Instrument1.TickType2.ToString])
            end
            else
              CellText := '';
          COL_INSTRUMENT2:
            if (Data^.Qualifier.TypeCondition = tcRealtime) then
            begin
              if (Data^.Qualifier.Instrument2.SokidInfo.SecurityType = 'OPT') or
                 (Data^.Qualifier.Instrument2.SokidInfo.SecurityType = 'FUT') then
                CellText := Format(rsInstrument, [Data^.Qualifier.Instrument2.SokidInfo.LocalSymbol,
                                                  Data^.Qualifier.Instrument2.SokidInfo.ContractId])
              else
                CellText := Format(rsInstrument, [Data^.Qualifier.Instrument2.SokidInfo.Symbol,
                                                  Data^.Qualifier.Instrument2.SokidInfo.ContractId])
            end
            else
              CellText := '';
          COL_INSTRUMENT2_VALUES:
            if (Data^.Qualifier.TypeCondition = tcRealtime) then
            begin
              if (Data^.Qualifier.Instrument2.TickType2 = ttNotSet) then
                CellText := Format('%.2f', [Data^.Qualifier.Instrument2.PriceValue1])
              else
                CellText := Format(rsValues, [Data^.Qualifier.Instrument2.PriceValue1,
                                              Data^.Qualifier.Instrument2.PriceValue2])
            end
            else
              CellText := '';
          COL_INSTRUMENT2_TICKTYPES:
            ...
            begin
              if (Data^.Qualifier.Instrument2.TickType2 = ttNotSet) then
                CellText := Data^.Qualifier.Instrument2.TickType1.ToString
              else
                CellText := Format(rsTickTypes, [Data^.Qualifier.Instrument2.TickType1.ToString,
                                                 Data^.Qualifier.Instrument2.TickType2.ToString])
            end
            else
              CellText := '';
        else
