...
uses
  ...
  Vcl.StdCtrls, Vcl.Buttons, Vcl.Graphics, Vcl.Samples.Spin, Vcl.ComCtrls, DebugWriter, DaModule, Candidate.Types,
  Candidate.FilterList, DaImages, Utils, Common.Types, Global.Types, Publishers;
{$ENDREGION}
...
type
  ...
  TCandidateEdit = class(TEdit, ICandidateControl)
  private
    FCode: string;
  ...
  TCandidateSpinEdit = class(TSpinEdit, ICandidateControl)
  private
    FCode: string;
    FFieldType: TFieldType;
    FVarName: string;
  ...
  TCandidateCheckBox = class(TCheckBox, ICandidateControl)
  private
    FCode: string;
    FFieldType: TFieldType;
    FVarName: string;
  ...
  TCandidateDateTimePicker = class(TDateTimePicker, ICandidateControl)
  private
    FCode: string;
    FFieldType: TFieldType;
    FVarName: string;
  ...
  TCandidateComboBox = class(TComboBox, ICandidateControl)
  private
    FCode         : string;
    FFieldType    : TFieldType;
    FVarName      : string;
...
begin
  Result := False;
  if (TfrmCandidateFilterList.ShowDocument(@FFilterList) = mrOk) then
  begin
...
var
  i: Integer;
  CandidateControl: ICandidateControl;
begin
  for i := 0 to Self.ControlCount - 1 do
    if Supports(Self.Controls[i], ICandidateControl, CandidateControl) then
      CandidateControl.EditValue := '';
end;
...
var
  ...
  CheckBox           : TCandidateCheckBox;
  ComboBox           : TCandidateComboBox;
  DateTimePicker     : TCandidateDateTimePicker;
  DisplayName        : string;
  Edit               : TCandidateEdit;
  i                  : Integer;
...
begin
  ...
  begin
    ...
    begin
      Edit := TCandidateEdit.Create(Self);
      Edit.Parent     := Self;
      Edit.Text       := '0';
    ...
    begin
      SpinEdit := TCandidateSpinEdit.Create(Self);
    ...
    begin
      CheckBox := TCandidateCheckBox.Create(Self);
    ...
    begin
      DateTimePicker := TCandidateDateTimePicker.Create(Self);
    ...
    begin
      ComboBox := TCandidateComboBox.Create(Self);
...
begin
  for i := 0 to Self.ControlCount - 1 do
    if Supports(Self.Controls[i], ICandidateControl, CandidateControl) and (CandidateControl.Code = Key) then
    begin
      CandidateControl.EditValue := Value;
      Break;
    end;
end;
...
begin
  ...
  for i := 0 to Self.ControlCount - 1 do
    ...
    begin
      case RangeState of
        rsLess:
          if CandidateControl.VarName = 'max' then
            FVariableList.AddOrSetValue(CandidateControl.Code, CandidateControl.EditValue);
        rsGreater:
          if CandidateControl.VarName = 'min' then
            FVariableList.AddOrSetValue(CandidateControl.Code, CandidateControl.EditValue);
      else
        FVariableList.AddOrSetValue(CandidateControl.Code, CandidateControl.EditValue);
      end;
        ...
        case Self.RangeState of
          rsLess:
            begin
              ...
              else if (CandidateControl.VarName = 'min') then
              begin
          ...
          rsRange:
            begin
              ...
              else if (CandidateControl.VarName = 'max') then
              begin
                Self.Controls[i].Width   := C_WIDTH_MAX;
...
function TCandidateEdit.GetCode: string;
begin
  Result := FCode;
end;

function TCandidateEdit.GetEditValue: string;
begin
...
function TCandidateEdit.GetFieldType: TFieldType;
begin
  Result := FFieldType;
end;

function TCandidateEdit.GetVarName: string;
begin
  Result := FVarName;
end;

procedure TCandidateEdit.SetCode(const Value: string);
begin
  FCode := Value;
end;

procedure TCandidateEdit.SetEditValue(const Value: string);
begin
...
procedure TCandidateEdit.SetFieldType(const Value: TFieldType);
begin
  FFieldType := Value;
end;

procedure TCandidateEdit.SetVarName(const Value: string);
begin
  FVarName := Value;
end;
...
function TCandidateSpinEdit.GetCode: string;
begin
  Result := FCode;
end;

function TCandidateSpinEdit.GetEditValue: string;
begin
  Result := Self.Value.ToString;
end;

function TCandidateSpinEdit.GetFieldType: TFieldType;
begin
  Result := FFieldType;
end;

function TCandidateSpinEdit.GetVarName: string;
begin
  Result := FVarName;
end;

procedure TCandidateSpinEdit.SetCode(const Value: string);
begin
  FCode := Value;
end;

procedure TCandidateSpinEdit.SetEditValue(const Value: string);
begin
...
procedure TCandidateSpinEdit.SetFieldType(const Value: TFieldType);
begin
  FFieldType := Value;
end;

procedure TCandidateSpinEdit.SetVarName(const Value: string);
begin
  FVarName := Value;
end;
...
constructor TCandidateComboBox.Create(AOwner: TComponent);
begin
  inherited;
  FDefaultIndex := -1;
...
function TCandidateComboBox.GetCode: string;
begin
  Result := FCode;
end;

function TCandidateComboBox.GetEditValue: string;
begin
...
function TCandidateComboBox.GetFieldType: TFieldType;
begin
  Result := FFieldType;
end;

function TCandidateComboBox.GetVarName: string;
begin
  Result := FVarName;
end;

procedure TCandidateComboBox.SetCode(const Value: string);
begin
  FCode := Value;
end;

procedure TCandidateComboBox.SetEditValue(const Value: string);
begin
  if Value.IsEmpty then
...
procedure TCandidateComboBox.SetFieldType(const Value: TFieldType);
begin
  FFieldType := Value;
end;

procedure TCandidateComboBox.SetVarName(const Value: string);
begin
  FVarName := Value;
end;
...
function TCandidateCheckBox.GetCode: string;
begin
  Result := FCode;
end;

function TCandidateCheckBox.GetEditValue: string;
begin
  Result := BoolToStr(Self.Checked, True).ToLower;
end;

function TCandidateCheckBox.GetFieldType: TFieldType;
begin
  Result := FFieldType;
end;

function TCandidateCheckBox.GetVarName: string;
begin
  Result := FVarName;
end;

procedure TCandidateCheckBox.SetCode(const Value: string);
begin
  FCode := Value;
end;

procedure TCandidateCheckBox.SetEditValue(const Value: string);
begin
  if Value.IsEmpty then
    Self.Checked := False
  else
...
procedure TCandidateCheckBox.SetFieldType(const Value: TFieldType);
begin
  FFieldType := Value;
end;

procedure TCandidateCheckBox.SetVarName(const Value: string);
begin
  FVarName := Value;
end;

{ TCandidateDateTimePicker }

function TCandidateDateTimePicker.GetCode: string;
begin
  Result := FCode;
end;

function TCandidateDateTimePicker.GetEditValue: string;
begin
  Result := FormatDateTime('YYYYMMDD', Self.Date);
end;

function TCandidateDateTimePicker.GetFieldType: TFieldType;
begin
  Result := FFieldType;
end;

function TCandidateDateTimePicker.GetVarName: string;
begin
  Result := FVarName;
end;

procedure TCandidateDateTimePicker.SetCode(const Value: string);
begin
  FCode := Value;
end;

procedure TCandidateDateTimePicker.SetEditValue(const Value: string);
begin
  if Value.IsEmpty then
...
procedure TCandidateDateTimePicker.SetFieldType(const Value: TFieldType);
begin
  FFieldType := Value;
end;

procedure TCandidateDateTimePicker.SetVarName(const Value: string);
begin
  FVarName := Value;
end;
