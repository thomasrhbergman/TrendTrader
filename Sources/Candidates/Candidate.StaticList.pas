...
uses
  ...
  BrokerHelperAbstr, Search.Instruments, DebugWriter, HtmlLib, Candidate.Types, CustomForms, MessageDialog,
...
type
  TfrmCandidateStaticList = class(TCustomForm)
    aCancel: TAction;
...
class function TfrmCandidateStaticList.ShowDocument(aRecordId: Integer): TModalResult;
var
  frmCandidateStaticList: TfrmCandidateStaticList;
begin
  frmCandidateStaticList := TfrmCandidateStaticList.Create(nil);
  try
    frmCandidateStaticList.FRecordID := aRecordId;
    frmCandidateStaticList.Initialize;
    Result := frmCandidateStaticList.ShowModal;
  finally
    frmCandidateStaticList.Denitialize;
    FreeAndNil(frmCandidateStaticList);
  end;
end;

procedure TfrmCandidateStaticList.FormCreate(Sender: TObject);
begin
  vstTree.NodeDataSize := SizeOf(TInstrumentData);
end;

procedure TfrmCandidateStaticList.Initialize;
begin
...
procedure TfrmCandidateStaticList.Denitialize;
begin
  fbqStaticLists.Close;
end;

procedure TfrmCandidateStaticList.vstTreeCompareNodes(Sender: TBaseVirtualTree; Node1, Node2: PVirtualNode; Column: TColumnIndex; var Result: Integer);
var
...
procedure TfrmCandidateStaticList.vstTreeDragAllowed(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex; var Allowed: Boolean);
begin
  Allowed := True;
end;

procedure TfrmCandidateStaticList.vstTreeDragDrop(Sender: TBaseVirtualTree; Source: TObject; DataObject: IDataObject; Formats: TFormatArray; Shift: TShiftState; Pt: TPoint; var Effect: Integer; Mode: TDropMode);
var
...
procedure TfrmCandidateStaticList.vstTreeDragOver(Sender: TBaseVirtualTree; Source: TObject; Shift: TShiftState; State: TDragState; Pt: TPoint; Mode: TDropMode; var Effect: Integer; var Accept: Boolean);
begin
  Accept := (Source is TVirtualStringTree);
end;

procedure TfrmCandidateStaticList.vstTreeDrawText(Sender: TBaseVirtualTree; TargetCanvas: TCanvas; Node: PVirtualNode; Column: TColumnIndex; const Text: string; const CellRect: TRect; var DefaultDraw: Boolean);
var
...
procedure TfrmCandidateStaticList.vstTreeEditing(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex; var Allowed: Boolean);
begin
  Allowed := True;
end;

procedure TfrmCandidateStaticList.vstTreeCreateEditor(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex; out EditLink: IVTEditLink);
begin
...
procedure TfrmCandidateStaticList.vstTreeFreeNode(Sender: TBaseVirtualTree; Node: PVirtualNode);
var
  Data: PInstrumentData;
begin
  Data := Node^.GetData;
...
procedure TfrmCandidateStaticList.vstTreeGetText(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType; var CellText: string);
var
  Data: PInstrumentData;
...
procedure TfrmCandidateStaticList.vstTreeNewText(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex; NewText: string);
var
  Data: PInstrumentData;
  ColumnsItem: TExtraColumns.TColumnsItem;
begin
...
procedure TfrmCandidateStaticList.LoadDetailList(const aRecordID: Integer);
resourcestring
...
procedure TfrmCandidateStaticList.SaveDetailList(const aRecordID: Integer);
resourcestring
...
procedure TfrmCandidateStaticList.aSaveExecute(Sender: TObject);
begin
  if (FRecordID > -1) then
  begin
...
procedure TfrmCandidateStaticList.aDeleteRowExecute(Sender: TObject);
resourcestring
...
procedure TfrmCandidateStaticList.aDeleteRowUpdate(Sender: TObject);
begin
  TAction(Sender).Enabled := not vstTree.IsEmpty;
end;

procedure TfrmCandidateStaticList.AddInstruments(aTarget: PVirtualNode; const aSource: TVirtualStringTree);
var
  Data: PInstrumentData;
  i{, j}: Integer;
  Node: PVirtualNode;
...
procedure TfrmCandidateStaticList.aDeleteListExecute(Sender: TObject);
begin
...
procedure TfrmCandidateStaticList.aShowSearchFormExecute(Sender: TObject);
begin
  TfrmSearchInstruments.ShowDocument(Self);
end;

procedure TfrmCandidateStaticList.aCancelExecute(Sender: TObject);
begin
  ModalResult := mrClose;
end;
