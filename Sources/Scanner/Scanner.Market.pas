...
uses
  ...
  DebugWriter, CustomForms, XmlFiles, Scanner.Types, Scanner.Filters, Scanner.AllFilters, VirtualTrees.Editors,
  Chart.Trade, Data.DB, HtmlLib, Scanner.MarketOpenSequence, Vcl.Printers,
  ...
  IABFunctions.MessageCodes, Scanner.DragDropOptions, System.Bindings.Expression, System.Bindings.ExpressionDefaults,
...
type
  TfrmScannerMarket = class(TCustomForm, IScanner,
    ...
    procedure aGetScannerParametersUpdate(Sender: TObject);
  ...
  private
    [weak] FScannerMarket: IScannerMarket;
    FColumnId: Integer;
    ...
    FScannerDataList: TDictionary<Integer, PInstrumentData>;
    FScannerParametersFile: string;
    FScanOptions: TScannerColumn;
    ...
    procedure AddInstrumentToScannerMain(const aKindAppend: TKindAppend);
    procedure AddInstrumentToStaticList(aDragDropOptions: TDragDropOptions);
  ...
  public
    class function ShowDocument(const aColumnId: Integer; const aScanOptions: TScannerColumn; const aScannerMarket: IScannerMarket): TModalResult;
...
resourcestring
  ...
  rsSave = 'Please save Market Scanner Sequence';
  rsWeight = 'Weight';

class function TfrmScannerMarket.ShowDocument(const aColumnId: Integer; const aScanOptions: TScannerColumn; const aScannerMarket: IScannerMarket): TModalResult;
begin
  Result := mrOk;
  with TfrmScannerMarket.Create(nil) do
  begin
    FColumnId := aColumnId;
    FScanOptions := aScanOptions;
    if Assigned(aScannerMarket) then
    begin
      FScannerMarket := aScannerMarket;
      FScannerMarket.IncMarketInstances;
    end;
    Initialize;
    CancelScan;
    CancelSubscribe;
    ...
    begin
      SetInstruments(GetRootInstrumentIndex(FScanOptions.ScannerMarket.InstrumentName));
...
procedure TfrmScannerMarket.FormCreate(Sender: TObject);
var
  i: Integer;
begin
  inherited;
  ...
  FScannerDataList := TDictionary<Integer, PInstrumentData>.Create;

  FScanOptions.Clear;
  FXmlFile := TXMLFile.Create;
  FScannerParametersFile := ExtractFilePath(Application.ExeName) + 'ScannerParameters.xml';

  for i := 0 to IABClient.Scanner.Count - 1 do
    OnScannerAdd(Self, IABClient.Scanner.Items[i].ScanId);
end;

procedure TfrmScannerMarket.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Deinitialize;
...
procedure TfrmScannerMarket.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
resourcestring
  rsQuestion = 'Do you want to change the column in AutoTrade?';
begin
  if Assigned(FScannerMarket) and (FColumnId > 0) then
    if (TMessageDialog.ShowQuestion(rsQuestion) = mrYes) then
      FScannerMarket.SetScanOptions(FColumnId, FScanOptions);
end;

procedure TfrmScannerMarket.FormDestroy(Sender: TObject);
begin
  if Assigned(FScannerMarket) then
    FScannerMarket.DecMarketInstances;

  FreeAndNil(FInstrumentList);
  FreeAndNil(FXmlFile);
  FreeAndNil(FScannerDataList);
...
procedure TfrmScannerMarket.Initialize;
var
...
procedure TfrmScannerMarket.Deinitialize;
begin
  CancelSubscribe;
  SaveParamsToXml;
  FScannerDataList.Clear;
  FScanList.Clear;
  CancelScan;
end;

procedure TfrmScannerMarket.LoadParamsFromXml;
begin

end;

procedure TfrmScannerMarket.SaveParamsToXml;
begin
...
procedure TfrmScannerMarket.edtAvailableFiltersChange(Sender: TObject);
var
  Data: PScanTypeData;
  Node: PVirtualNode;
begin
...
procedure TfrmScannerMarket.edtIsolateSearchChange(Sender: TObject);
var
  Data: PSokidInfo;
  Node: PVirtualNode;
begin
...
procedure TfrmScannerMarket.lbInstrumentsClick(Sender: TObject);
var
  i: Integer;
  InstrumentInfo: TInstrumentInfo;
begin
  ...
  begin
    for i := 0 to lbInstruments.Items.Count - 1 do
      ...
      begin
        ...
        FScannerDataList.Clear;
        FScanOptions.ScannerMarket.InstrumentIndex := i;
        FScanOptions.ScannerMarket.InstrumentName  := InstrumentInfo.Name;
        Break;
      end;
  end;
end;

procedure TfrmScannerMarket.SetRootInstrument;
begin
  for var i := 0 to lbInstruments.Items.Count - 1 do
    if lbInstruments.Selected[i] and Assigned(lbInstruments.Items.Objects[i]) then
    begin
      FScanOptions.ScannerMarket.InstrumentName := TInstrumentInfo(lbInstruments.Items.Objects[i]).Name;
      FScanOptions.ScannerMarket.InstrumentIndex := i;
      Break;
    end;
end;

procedure TfrmScannerMarket.ParseInstrumentInfo(const aInstrument: TInstrumentInfo);
var
...
procedure TfrmScannerMarket.ParseLocation(aXmlNode: IXMLDOMNode; aParentNode: PVirtualNode);
var
  ChildNode: IXMLDOMNode;
  i: Integer;
...
procedure TfrmScannerMarket.ParseScanType(aXmlNode: IXMLDOMNode; aParseScanType: string);
var
  i: Integer;
  ChildNode: IXMLDOMNode;
  Data: PScanTypeData;
...
procedure TfrmScannerMarket.ParseScanTypeList(const aInstruments: string);
var
  i: Integer;
...
procedure TfrmScannerMarket.ParseFilterList;
var
  ChildNode: IXMLDOMNode;
  CurrentNode: IXMLDOMNode;
...
procedure TfrmScannerMarket.ParseFilter(aXmlNode: IXMLDOMNode);
var
  i: Integer;
  ChildNode: IXMLDOMNode;
  Data: TFilterData;
...
function TfrmScannerMarket.ParseLocationInfo(aXmlNode: IXMLDOMNode; aParentNode: PVirtualNode): PVirtualNode;
var
  i: Integer;
  ChildNode: IXMLDOMNode;
  Data: PLocationData;
begin
  ...
  begin
    ...
    for var item in FScanOptions.ScannerMarket.MarketLocation do
...
procedure TfrmScannerMarket.RankingSum;
var
...
procedure TfrmScannerMarket.RecalcSortIndex;
var
  Data: PInstrumentData;
  Node: PVirtualNode;
  i: Integer;
...
procedure TfrmScannerMarket.OnScannerData(Sender: TObject; Scan: TIABScan);
    ...
    try
      ...
      begin
        ...
        begin
          ...
          FScannerDataList.Remove(Data^.Id);
      ...
      for i := 0 to Scan.Count - 1 do
      begin
        if (FScannerDataList.ContainsKey(Scan.Items[i].ContractId)) then
          Data := FScannerDataList.Items[Scan.Items[i].ContractId]
        else
        begin
          ...
          FScannerDataList.Add(Scan.Items[i].ContractId, Data);
        end;
...
procedure TfrmScannerMarket.OnScannerParam(Sender: TObject; Parameters: string);
begin
  inherited;
...
procedure TfrmScannerMarket.UpdateStatus(const aPanelNum: Byte; const aInfo: string);
begin
  sbMain.Panels[aPanelNum].Text := aInfo;
  sbMain.Refresh;
end;

procedure TfrmScannerMarket.OnPriceChange(Sender: TObject; Id: Integer; TickType: TIABTickType; Value: Currency; TimeStamp: TDateTime);
var
...
procedure TfrmScannerMarket.OnError(Sender: TObject; TempId, ErrorCode: Integer; ErrorMsg: string);
resourcestring
...
procedure TfrmScannerMarket.OnInstrumentSpecDetails(Sender: TObject; Index: Integer);
var
...
begin
  ...
  begin
    ...
    if FScannerDataList.ContainsKey(SokidInfo.ContractId) then
    begin
      Data := FScannerDataList.Items[SokidInfo.ContractId];
...
function TfrmScannerMarket.GetIdentityName: string;
begin
  Result := C_IDENTITY_NAME;
end;

function TfrmScannerMarket.GetInstance: TObject;
begin
  Result := Self;
end;

procedure TfrmScannerMarket.GetInstrumentSpecs(aData: PInstrumentData);
var
  Order: TIABOrder;
begin
...
procedure TfrmScannerMarket.ParseXmlParams(const aXmlText: string = '');
var
  ChildNode: IXMLDOMNode;
  CurrentNode: IXMLDOMNode;
  i: Integer;
...
begin
  ...
  else if FileExists(FScannerParametersFile) then
    FXmlFile.LoadFromFile(FScannerParametersFile);
  ...
  FXmlFile.CurrentSection := '';
  FXmlFile.SaveToFile(FScannerParametersFile);
end;

procedure TfrmScannerMarket.aAddAllExecute(Sender: TObject);
begin
  AddInstrumentToScannerMain(kaAddAll);
end;

procedure TfrmScannerMarket.aInformationDialogExecute(Sender: TObject);
begin
  TInformationDialog.ShowMessage(FScanOptions.ToList.Replace(sLineBreak, '<br>'), 'ScannerMarket');
end;

procedure TfrmScannerMarket.aIntersectionExecute(Sender: TObject);
begin
  AddInstrumentToScannerMain(kaIntersection);
end;

procedure TfrmScannerMarket.aReduceExecute(Sender: TObject);
begin
  AddInstrumentToScannerMain(kaReduce);
end;

procedure TfrmScannerMarket.aUpdateExecute(Sender: TObject);
begin
  AddInstrumentToScannerMain(kaUpdate);
end;

procedure TfrmScannerMarket.CancelScan;
begin
  if IABClient.Connected and (FScanOptions.ScanId > 0) then
    IABClient.CancelScan(FScanOptions.ScanId);
end;

procedure TfrmScannerMarket.CancelSubscribe;
begin
  for var DataId in FInstrumentList.Keys do
    TIABMarket.CancelMarketData(DataId);
end;

procedure TfrmScannerMarket.cbUseExtraFilterClick(Sender: TObject);
begin
  FUseExtraFilters := cbUseExtraFilter.Checked;
end;

function TfrmScannerMarket.GetScanCriteria(const aScanCode: string): TIABScanCriteria;
var
  Data          : PLocationData;
...
begin
  ...
  begin
    ...
    begin
      ...
      ScanCriteria.ScannerSettingPairs      := FScanOptions.GlobalScanCriteria.ScannerSettingPairs;
...
procedure TfrmScannerMarket.NewScan;
resourcestring
...
function TfrmScannerMarket.GetLocationCode(const aNode: PVirtualNode): string;
var
...
procedure TfrmScannerMarket.GUIToScanOptions;
begin
  ...
  FScanOptions.ScannerMarket.AutoRefresh      := cbAutoRefresh.Checked;
  FScanOptions.ScannerMarket.AvailableFilters := edtAvailableFilters.Text;
  FScanOptions.ScannerMarket.UseExtraFilters  := UseExtraFilters;
end;

procedure TfrmScannerMarket.ScanOptionsToGUI;
begin
  cbAutoRefresh.Checked      := FScanOptions.ScannerMarket.AutoRefresh;
  edCurrentSequenceName.Text := FScanOptions.Name;
  edtAvailableFilters.Text   := FScanOptions.ScannerMarket.AvailableFilters;
  edtStep.ValueFloat         := FScanOptions.Step;
  edtWeight.ValueFloat       := FScanOptions.Weight;
  seMaxRows.Value            := FScanOptions.MaxRows;
  UseExtraFilters            := FScanOptions.ScannerMarket.UseExtraFilters;
  if Assigned(FScannerMarket) then
  begin
    lblAutoTradesName.Caption := 'Parent AutoTrades Name: ' + FScannerMarket.GetAutoTradeInfo.Name;
    lblInstanceNum.Caption := Concat('Parent Instance Num: ', Abs(FScannerMarket.GetAutoTradeInfo.InstanceNum).ToString);
    if (FScannerMarket.GetAutoTradeInfo.InstanceNum > 0) then
...
procedure TfrmScannerMarket.seMaxRowsChange(Sender: TObject);
begin
  if Showing then
  begin
    FScanOptions.MaxRows := seMaxRows.Value;
...
procedure TfrmScannerMarket.aNewScanExecute(Sender: TObject);
begin
...
procedure TfrmScannerMarket.aNewScanUpdate(Sender: TObject);
begin
  TAction(Sender).Enabled := IABClient.Connected;
end;

procedure TfrmScannerMarket.aAddAllUpdate(Sender: TObject);
begin
  TAction(Sender).Enabled := not vstInstruments.IsEmpty;
end;

procedure TfrmScannerMarket.ReloadFilterList(aFilters: string);
var
  i: Integer;
...
procedure TfrmScannerMarket.aAddFilterExecute(Sender: TObject);
begin
  FFiltersController.AddOrReplaceFilters;
end;

procedure TfrmScannerMarket.aClearAllFiltersExecute(Sender: TObject);
begin
  FFiltersController.ClearAllFilters;
end;

procedure TfrmScannerMarket.aClearAvailableFiltersExecute(Sender: TObject);
begin
  edtAvailableFilters.Text := '';
  edtAvailableFiltersChange(nil);
end;

procedure TfrmScannerMarket.aClearIsolateSearchTextExecute(Sender: TObject);
begin
  edtIsolateSearch.Text := '';
end;

procedure TfrmScannerMarket.aColumnSettingsExecute(Sender: TObject);
begin
  TStoreHelper.ShowColumnSettings(vstInstruments, GetIdentityName + '.vstInstruments', C_FIXED_COLUMN_INDEX);
end;

procedure TfrmScannerMarket.aRemoveAllExecute(Sender: TObject);
begin
  FFiltersController.RemoveAllFilters;
end;

procedure TfrmScannerMarket.ShowTradeChart(aNode: PVirtualNode);
var
  Data: PInstrumentData;
  i: Integer;
...
procedure TfrmScannerMarket.aOpenExecute(Sender: TObject);
var
  RecordId: Integer;
begin
  RecordId := TfrmScannerMarketOpenSequence.GetScanRecordID;
  if (RecordId > 0) then
    Open(RecordId);
end;

procedure TfrmScannerMarket.SetInstruments(const aIndex: Integer);
var
  Node: PVirtualNode;
  ScanCode: string;
  ScanTypeData: PScanTypeData;
...
procedure TfrmScannerMarket.ReadFilters;
var
...
begin
  FFiltersController.RemoveAllFilters;
  for var i := Low(FScanOptions.ScannerMarket.FiltersArray) to High(FScanOptions.ScannerMarket.FiltersArray) do
  begin
    Id := FScanOptions.ScannerMarket.FiltersArray[i].Id;
    if not Id.IsEmpty then
    begin
      ArrVar := FScanOptions.ScannerMarket.FiltersArray[i].Variables.Split([C_SEPARATOR]);
      if FFiltersController.AddFilter(Id) then
      begin
        if InRange(FScanOptions.ScannerMarket.FiltersArray[i].RangeState, Ord(Low(TRangeState)), Ord(High(TRangeState))) then
          FFiltersController.Items[Id].RangeState := TRangeState(FScanOptions.ScannerMarket.FiltersArray[i].RangeState)
        else
...
procedure TfrmScannerMarket.WriteFilters;
var
  Pair: TPair<string, string>;
  FilterControl: TFilterControl;
  Variables: string;
  i: Integer;
begin
  FScanOptions.ScannerMarket.FiltersArray := [];
  i := 0;
  SetLength(FScanOptions.ScannerMarket.FiltersArray, FFiltersController.Count);
  for FilterControl in FFiltersController.Values do
    if Assigned(FilterControl) then
    begin
      FScanOptions.ScannerMarket.FiltersArray[i].Id         := FilterControl.Id;
      FScanOptions.ScannerMarket.FiltersArray[i].RangeState := Ord(FilterControl.RangeState);
      Variables := '';
      for Pair in FilterControl.VariableList do
        Variables := Variables + Pair.Key + '=' + Pair.Value + C_SEPARATOR;
      FScanOptions.ScannerMarket.FiltersArray[i].Variables := Variables;
      Inc(i);
    end;
end;

procedure TfrmScannerMarket.ReadExtraFilters;
...
begin
  ...
  FScanOptions.GlobalScanCriteria.ScannerSettingPairs      := FScanOptions.ScanCriteria.ScannerSettingPairs;
...
procedure TfrmScannerMarket.WriteExtraFilters;
...
//  if UseExtraFilters then
  begin
    ...
    FScanOptions.ScanCriteria.ScannerSettingPairs      := FScanOptions.GlobalScanCriteria.ScannerSettingPairs;
...
procedure TfrmScannerMarket.Open(aSequenceRecordId: Integer);
    ...
    try
      ...
      try
        ...
        begin
          ...
          FScanOptions.ScannerMarket.RecordId := aSequenceRecordId;
          edCurrentSequenceName.Text          := FScanOptions.Name;
        end;
      except
        on E: Exception do
...
begin
  if (aSequenceRecordId > 0) then
    LoadDataFromDB;
  ScanOptionsToGUI;
  SetInstruments(FScanOptions.ScannerMarket.InstrumentIndex);
  ReadFilters;
  ReadExtraFilters;
...
function TfrmScannerMarket.GetRootInstrumentIndex(aRootInstrumentName: string): Integer;
begin
  Result := -1;
  for var i := 0 to lbInstruments.Items.Count - 1 do
...
procedure TfrmScannerMarket.aSaveAsExecute(Sender: TObject);
var
  NewName: string;
begin
  ...
  begin
    ...
    FScanOptions.ScannerMarket.RecordId := -1;
    Save;
  end;
end;

procedure TfrmScannerMarket.aSaveAsUpdate(Sender: TObject);
begin
  TAction(Sender).Enabled := FScanOptions.ScannerMarket.RecordId > 0;
end;

procedure TfrmScannerMarket.aSaveExecute(Sender: TObject);
begin
  if CheckData then
    Save;
end;

procedure TfrmScannerMarket.Save;
resourcestring
...
begin
  ...
  try
    ...
    if (FScanOptions.ScannerMarket.RecordId <= 0) then
    begin
      FScanOptions.ScannerMarket.RecordId := DMod.GetNextValue('GEN_SCAN_MARKET_ID');
    ...
    WriteFilters;
    if FScanOptions.Name.IsEmpty then
      FScanOptions.Name := 'Scanner Market sequence nr' + FScanOptions.ScannerMarket.RecordId.ToString;
    FScanOptions.ScanCriteria := GetScanCriteria(FScanOptions.ScanCriteria.ScanCode);
    Query.ParamByName('ID').AsInteger     := FScanOptions.ScannerMarket.RecordId;
...
procedure TfrmScannerMarket.aShowDragDropOptionsExecute(Sender: TObject);
begin
  TfrmDragDropOptions.ShowDocument;
end;

procedure TfrmScannerMarket.aShowTradeChartExecute(Sender: TObject);
begin
  if Assigned(vstInstruments.FocusedNode) then
    ShowTradeChart(vstInstruments.FocusedNode);
end;

procedure TfrmScannerMarket.aShowTradeChartUpdate(Sender: TObject);
begin
  TAction(Sender).Enabled := IABClient.Connected and not vstInstruments.IsEmpty;
end;

procedure TfrmScannerMarket.AddInstrumentToStaticList(aDragDropOptions: TDragDropOptions);
var
  ColumnItems: TExtraColumns.TColumnsItem;
...
begin
  ...
  else if Assigned(FScannerMarket) and (DialogMode <> dmView) and (vstInstruments.SelectedCount > 0) then
  begin
    Index := 0;
    ...
    FScannerMarket.AddInstrument(@Instruments, ColumnsInfo);
  end;
end;

procedure TfrmScannerMarket.AddInstrumentToScannerMain(const aKindAppend: TKindAppend);
var
  ColumnItems: TExtraColumns.TColumnsItem;
  ColumnsInfo: TColumnsInfo;
  Data: PInstrumentData;
...
//  else if (edtStep.ValueFloat <= 0) then
//    TMessageDialog.ShowWarning(Format(rcRequiredValue, ['Step']))
  else if Assigned(FScannerMarket) and (DialogMode <> dmView) and (not vstInstruments.IsEmpty) then
  begin
    GUIToScanOptions;
    Index := 0;
    ...
    ColumnsInfo := TColumnsInfo.Create(stScannerMarket);
    ColumnsInfo.Weight := FScanOptions.Weight;
    SetRootInstrument;
    ColumnsInfo.ScannerColumn.FromList(FScanOptions.ToList);
    ColumnsInfo.ScannerColumn.Name := Format(rsColumnName, [FScanOptions.ScannerMarket.RecordId, FScanOptions.Name]);
    ColumnsInfo.KindAppend := aKindAppend;
    FScannerMarket.AddInstrument(@Instruments, ColumnsInfo);
  end;
end;

procedure TfrmScannerMarket.SubscribeMarketIABData(const aConId: Integer; const aNode: PVirtualNode);
var
  Order: TIABOrder;
  Data: PInstrumentData;
  MarketData: string;
...
procedure TfrmScannerMarket.aExtraFilterExecute(Sender: TObject);
begin
  if (TfrmScannerAllFilters.ShowDocument(@FScanOptions.GlobalScanCriteria) = mrOk) then
    WriteExtraFilters;
end;

procedure TfrmScannerMarket.GetScannerParameters;
const
  cFileSize = 1000000;
begin
  if FileExists(FScannerParametersFile) and (FileSizeByName(FScannerParametersFile) > cFileSize) then
    ParseXmlParams
  else if IABClient.Connected then
    IABClient.Scanner.GetScannerParameters;
end;

function TfrmScannerMarket.GetUseExtraFilters: Boolean;
begin
  Result := FUseExtraFilters;
end;

procedure TfrmScannerMarket.SetUseExtraFilters(const Value: Boolean);
begin
  FUseExtraFilters := Value;
  cbUseExtraFilter.Checked := Value;
end;

procedure TfrmScannerMarket.aGetScannerParametersUpdate(Sender: TObject);
begin
  TAction(Sender).Enabled := IABClient.Connected;
end;

function TfrmScannerMarket.CheckData: Boolean;
var
  Msg: string;
  ScanTypeData: PScanTypeData;
begin
...
procedure TfrmScannerMarket.ClearInstruments;
var
  i: Integer;
begin
  for i := 0 to lbInstruments.Items.Count - 1 do
...
procedure TfrmScannerMarket.vstLocationChecked(Sender: TBaseVirtualTree; Node: PVirtualNode);
var
  Data: PLocationData;
  IsExists: Boolean;
begin
  ...
  begin
    Data := Node^.GetData;
    IsExists := False;
    for var i := Low(FScanOptions.ScannerMarket.MarketLocation) to High(FScanOptions.ScannerMarket.MarketLocation) do
      if (FScanOptions.ScannerMarket.MarketLocation[i].Tag.ToUpper = Data^.LocationCode) then
      begin
        IsExists := True;
        FScanOptions.ScannerMarket.MarketLocation[i].Tag   := Data^.LocationCode;
        FScanOptions.ScannerMarket.MarketLocation[i].Value := Ord(Node.CheckState).ToString;
        Break;
      end;
      if not IsExists then
      begin
        SetLength(FScanOptions.ScannerMarket.MarketLocation, Length(FScanOptions.ScannerMarket.MarketLocation) + 1);
        FScanOptions.ScannerMarket.MarketLocation[Length(FScanOptions.ScannerMarket.MarketLocation) - 1].Tag   := Data^.LocationCode;
        FScanOptions.ScannerMarket.MarketLocation[Length(FScanOptions.ScannerMarket.MarketLocation) - 1].Value := Ord(Node.CheckState).ToString;
      end;
  end;
end;

procedure TfrmScannerMarket.vstLocationFreeNode(Sender: TBaseVirtualTree; Node: PVirtualNode);
var
  Data: PLocationData;
begin
  Data := Node^.GetData;
...
procedure TfrmScannerMarket.vstLocationGetText(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType; var CellText: string);
var
  Data: PLocationData;
begin
...
procedure TfrmScannerMarket.vstScanTypeChange(Sender: TBaseVirtualTree; Node: PVirtualNode);
var
  Data: PScanTypeData;
begin
...
procedure TfrmScannerMarket.vstScanTypeCompareNodes(Sender: TBaseVirtualTree; Node1, Node2: PVirtualNode; Column: TColumnIndex; var Result: Integer);
var
...
procedure TfrmScannerMarket.vstScanTypeDblClick(Sender: TObject);
begin
  aNewScanExecute(nil);
end;

procedure TfrmScannerMarket.vstScanTypeFreeNode(Sender: TBaseVirtualTree; Node: PVirtualNode);
var
  Data: PScanTypeData;
begin
  Data := Node^.GetData;
...
procedure TfrmScannerMarket.vstScanListCompareNodes(Sender: TBaseVirtualTree; Node1, Node2: PVirtualNode; Column: TColumnIndex; var Result: Integer);
var
...
procedure TfrmScannerMarket.vstScanListFreeNode(Sender: TBaseVirtualTree; Node: PVirtualNode);
var
  Data: PScanItem;
begin
  Data := Node^.GetData;
...
procedure TfrmScannerMarket.vstScanListGetNodeDataSize(Sender: TBaseVirtualTree; var NodeDataSize: Integer);
begin
  NodeDataSize := SizeOf(TScanItem);
end;

procedure TfrmScannerMarket.vstScanListGetText(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType; var CellText: string);
var
  Data: PScanItem;
begin
  Data := Node^.GetData;
...
procedure TfrmScannerMarket.OnScannerAdd(Sender: TObject; ScanId: Integer);
var
  Data: PScanItem;
  Node: PVirtualNode;
  i: Integer;
...
procedure TfrmScannerMarket.OnScannerCancel(Sender: TObject; ScanId: Integer);
var
  Node: PVirtualNode;
begin
  vstScanList.BeginUpdate;
...
procedure TfrmScannerMarket.aCancelScanExecute(Sender: TObject);
var
  Data: PScanItem;
  Node: PVirtualNode;
begin
...
procedure TfrmScannerMarket.vstScanTypeGetText(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType; var CellText: string);
var
  Data: PScanTypeData;
begin
  Data := Node^.GetData;
...
procedure TfrmScannerMarket.vstInstrumentsCompareNodes(Sender: TBaseVirtualTree; Node1, Node2: PVirtualNode; Column: TColumnIndex; var Result: Integer);
var
...
procedure TfrmScannerMarket.vstInstrumentsCreateEditor(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex; out EditLink: IVTEditLink);
begin
...
procedure TfrmScannerMarket.vstInstrumentsDragAllowed(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex; var Allowed: Boolean);
begin
  Allowed := vstInstruments.SelectedCount > 0;
end;

procedure TfrmScannerMarket.vstInstrumentsDrawText(Sender: TBaseVirtualTree; TargetCanvas: TCanvas; Node: PVirtualNode; Column: TColumnIndex; const Text: string; const CellRect: TRect; var DefaultDraw: Boolean);
var
  Data: PInstrumentData;
begin
  if Assigned(Node) then
...
procedure TfrmScannerMarket.vstInstrumentsEdited(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex);
begin
  if Assigned(Node) then
  begin
...
procedure TfrmScannerMarket.vstInstrumentsEditing(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex; var Allowed: Boolean);
begin
  Allowed := True;
end;

procedure TfrmScannerMarket.vstInstrumentsEndDrag(Sender, Target: TObject; X, Y: Integer);
var
  DragDropOptions: TDragDropOptions;
begin
  if Assigned(FScannerMarket) and (Target = FScannerMarket.GetMainTree) then
  begin
    DragDropOptions := TfrmDragDropOptions.GetDragDropOptions;
    if not DragDropOptions.RememberLastOption then
    begin
      if (TfrmDragDropOptions.ShowDocument = mrOk) then
        TfrmDragDropOptions.LoadParamsFromXml(DragDropOptions)
      else
        Exit;
    end;
    if (DragDropOptions.SourceType = stScannerMarket) then
      AddInstrumentToScannerMain(kaAddAll)
    else
...
procedure TfrmScannerMarket.vstInstrumentsFreeNode(Sender: TBaseVirtualTree; Node: PVirtualNode);
var
  Data: PInstrumentData;
begin
  Data := Node^.GetData;
...
procedure TfrmScannerMarket.vstInstrumentsGetText(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType; var CellText: string);
...
procedure TfrmScannerMarket.aPrintExecute(Sender: TObject);
begin
  Printer.Orientation := poLandscape;
  vstInstruments.Print(Printer, True);
end;

procedure TfrmScannerMarket.aExportToCSVExecute(Sender: TObject);
begin
  TExcelExportHelper.ExportToCSV(vstInstruments, Self.Caption);
end;

procedure TfrmScannerMarket.aExportToExcelExecute(Sender: TObject);
begin
  TExcelExportHelper.ExportToExcel(vstInstruments, Self.Caption);
end;

function TfrmScannerMarket.GetIsolateColumnValue(aColumn: Integer; aData: PSokidInfo): string;
begin
  case aColumn of
    COL_EXCLUDE_NAME:
      Result := aData^.Name;
...
procedure TfrmScannerMarket.vstIsolateCompareNodes(Sender: TBaseVirtualTree; Node1, Node2: PVirtualNode; Column: TColumnIndex; var Result: Integer);
var
...
procedure TfrmScannerMarket.vstIsolateDragAllowed(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex; var Allowed: Boolean);
begin
  Allowed := True;
end;

procedure TfrmScannerMarket.vstIsolateDragDrop(Sender: TBaseVirtualTree; Source: TObject; DataObject: IDataObject; Formats: TFormatArray; Shift: TShiftState; Pt: TPoint; var Effect: Integer; Mode: TDropMode);
var
  NewData: PSokidInfo;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          if FScannerDataList.ContainsKey(NewData^.ContractId) then
            FScannerDataList.Items[NewData^.ContractId].Isolate := 1;
        end;
...
procedure TfrmScannerMarket.vstIsolateDragOver(Sender: TBaseVirtualTree; Source: TObject; Shift: TShiftState; State: TDragState; Pt: TPoint; Mode: TDropMode; var Effect: Integer; var Accept: Boolean);
begin
  Accept := Source = vstInstruments;
end;

procedure TfrmScannerMarket.vstIsolateFreeNode(Sender: TBaseVirtualTree; Node: PVirtualNode);
var
  Data: PSokidInfo;
begin
  Data := Node^.GetData;
...
procedure TfrmScannerMarket.vstIsolateGetText(Sender: TBaseVirtualTree; Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType; var CellText: string);
var
  Data: PSokidInfo;
begin
  Data := Node^.GetData;
...
procedure TfrmScannerMarket.aIsolateColumnSettingsExecute(Sender: TObject);
begin
  TStoreHelper.ShowColumnSettings(vstIsolate, GetIdentityName, 0);
end;

procedure TfrmScannerMarket.aIsolateDeleteExecute(Sender: TObject);
var
  SokidInfo: TSokidInfo;
  Data: PSokidInfo;
  Node: PVirtualNode;
begin
  ...
  begin
    ...
    begin
      ...
      if FScannerDataList.ContainsKey(Data^.ContractId) then
        FScannerDataList.Items[Data^.ContractId].Isolate := 0;
    end;
  end;
end;

procedure TfrmScannerMarket.FillIsolate;
var
  SokidInfo: TSokidInfo;
  Data: PSokidInfo;
  Node: PVirtualNode;
