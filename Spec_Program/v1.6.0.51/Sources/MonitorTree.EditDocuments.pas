...
uses
  ...
  MessageDialog, Monitor.Types, Edit.OrderGroup, Edit.Condition, Edit.Algos, Edit.OrderTemplate, Edit.Factor,
  Qualifiers.Edit, AutoTrades.Edit, Edit.OrderGroupSet, Edit.OrderIB, Edit.OrderNN, Common.Types,
  Qualifiers.EditCondition, MonitorTree.Document;
{$ENDREGION}
    ...
    case Data^.DocType of
      ...
      ntOrder:
        begin
          if UseInTemplate then
          begin
            if TfrmOrderTemplate.ShowDocument(Data^.OrderDoc, TTreeDocument.GetOrderSubordination(aTree, Node)) = mrOk then
