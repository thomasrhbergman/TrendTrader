...
begin
  ...
  begin
    ...
    case TTreeDocument.GetDocType(TVirtualStringTree(Source), MoveNode) of
      ...
      ntAutoTrade:
        Accept := TTreeDocument.GetDocType(Sender, ToNode) in [ntQualifierCondition, ntUnknow];
      ntOrderGroupSet:
  ...
  begin
    ...
    begin
      ...
      if Assigned(Data) then
        case Data^.DocType of
          ntQualifier:
            Result := Data^.Qualifier.Description;
          ntQualifierCondition:
            Result := Data^.QualifierCondition.Name;
          ntAutoTrade:
...
begin
  ...
  if Result then
    case Self.DocType of
      ...
      ntQualifierCondition:
        Result := Self.RecordId = aTreeData.QualifierCondition.RecordId;
      ntAutoTrade:
