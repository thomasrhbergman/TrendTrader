...
uses
  ...
  DaImages, Edit.OrderGroup, Edit.Condition, Edit.Algos, Edit.OrderTemplate, Edit.Factor, Qualifiers.Types, ArrayHelper,
  Qualifiers.Edit, Qualifiers, AutoTrades.Edit, Edit.OrderGroupSet, Vcl.ComCtrls, InformationDialog, Frame.DocumentsTree,
  VirtualTrees.ExportHelper, Common.Types, Monitor.Info, System.Threading, CustomDockForm, Qualifiers.EditCondition,
...
//      if (ParentData.OrderDoc.ExtendedOptions.Subordination in [suUnknow, suMotherOrder]) then
      begin
        ...
        if (TfrmOrderTemplate.ShowDocument(Data.OrderDoc, TTreeDocument.GetOrderSubordination(vstTree, Node)) <> mrOk) then
...
begin
  inherited;
  TAction(Sender).Visible := TTreeDocument.GetDocType(vstTree, vstTree.FocusedNode) in [ntAutoTrade, ntQualifierCondition];
  TAction(Sender).Enabled := TAction(Sender).Visible;
end;
...
begin
  inherited;
  TAction(Sender).Visible := TTreeDocument.GetDocType(vstTree, vstTree.FocusedNode) in [ntQualifier, ntQualifierCondition];
  TAction(Sender).Enabled := TAction(Sender).Visible;
end;
...
begin
  ...
  begin
    ...
    case Data^.DocType of
      ntQualifier:
        if (TfrmQualifiersEdit.ShowDocument(Node) = mrOk) then
        begin
      ...
      ntQualifierCondition:
        if (TfrmQualifierConditionEdit.ShowDocument(Data^.QualifierCondition, dmUpdate) = mrOk) then
...
begin
    ...
    begin
      ...
      begin
        ...
        case Data^.DocType of
          ...
          ntQualifierCondition:
            Data^.QualifierCondition.RecordId := -1;
          ntAutoTrade:
  ...
  try
    ...
    case SourceData^.DocType of
      ...
      ntQualifierCondition:
        begin
          NewNode := TTreeDocument.CreateQualifierCondition(aTargetNode, vstTree);
          NewData := NewNode^.GetData;
