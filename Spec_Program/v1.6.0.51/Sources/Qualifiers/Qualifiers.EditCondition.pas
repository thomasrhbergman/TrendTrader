...
type
  TfrmQualifierConditionEdit = class(TCustomForm, IUpdateFeeds)
  ...
  private
    FQualifierItem: TQualifierCondition;
  ...
  public
    class function ShowDocument(var aQualifierItem: TQualifierCondition; aDialogMode: TDialogMode): TModalResult;
...
class function TfrmQualifierConditionEdit.ShowDocument(var aQualifierItem: TQualifierCondition; aDialogMode: TDialogMode): TModalResult;
begin
  with TfrmQualifierConditionEdit.Create(nil) do
  try
...
procedure TfrmQualifierConditionEdit.FormCreate(Sender: TObject);
begin
  FQualifierItem := TQualifierCondition.Create;
  TPublishers.FeedPublisher.Subscribe(Self);
end;

procedure TfrmQualifierConditionEdit.FormDestroy(Sender: TObject);
begin
  FreeAndNil(FQualifierItem);
  TPublishers.FeedPublisher.Unsubscribe(Self);
end;

procedure TfrmQualifierConditionEdit.Initialize;
resourcestring
  rsCaption = '%s (v.%s)';
begin
  ...
  if FQualifierItem.Name.IsEmpty then
    edtName.Text := 'Qualifier Condition'
  else
  ...
  case DialogMode of
    dmInsert:
      begin
        ...
        Self.Caption := Format(rsCaption, ['New Qualifier Condition', General.ModuleVersion]);
      end;
    dmUpdate:
      Self.Caption := Format(rsCaption, ['Edit Qualifier Condition', General.ModuleVersion]);
  end;
end;

procedure TfrmQualifierConditionEdit.SokidInfoToGUI(Sender: TObject; aSokidInfo: TSokidInfo);
begin
...
procedure TfrmQualifierConditionEdit.QualifierInstrumentToGUI(Sender: TObject; aQualifierInstrument: TQualifierInstrument);
begin
  if (Sender = pnlInstrument1) then
  begin
...
procedure TfrmQualifierConditionEdit.Denitialize;
begin
...
procedure TfrmQualifierConditionEdit.OnInstrumentDragDrop(Sender, Source: TObject; X, Y: Integer);
var
...
procedure TfrmQualifierConditionEdit.OnInstrumentDragOver(Sender, Source: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
  Accept := Assigned(Source) and (Source is TVirtualStringTree);
end;

procedure TfrmQualifierConditionEdit.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if Assigned(frmSearchInstruments) and (frmSearchInstruments.StickForm = Self) then
    frmSearchInstruments.Close;
end;

procedure TfrmQualifierConditionEdit.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  CanClose := True;
...
procedure TfrmQualifierConditionEdit.aShowSearchInstrumentsExecute(Sender: TObject);
begin
  TfrmSearchInstruments.ShowDocument(Self);
end;

procedure TfrmQualifierConditionEdit.cbInstrument1IBValue1Change(Sender: TObject);
begin
  if (cbInstrument1IBValue1.ItemIndex > -1) then
    FQualifierItem.Instrument1.TickType1 := TIABTickType(cbInstrument1IBValue1.ItemIndex)
  else
...
procedure TfrmQualifierConditionEdit.cbInstrument1IBValue2Change(Sender: TObject);
begin
  if (cbInstrument1IBValue2.ItemIndex > -1) then
    FQualifierItem.Instrument1.TickType2 := TIABTickType(cbInstrument1IBValue2.ItemIndex)
  else
...
procedure TfrmQualifierConditionEdit.cbTypeOperation1Change(Sender: TObject);
begin
  if (cbTypeOperation1.ItemIndex > -1) then
    FQualifierItem.Instrument1.TypeOperation := TTypeOperation(cbTypeOperation1.ItemIndex)
  else
...
procedure TfrmQualifierConditionEdit.cbTypeOperation2Change(Sender: TObject);
begin
  if (cbTypeOperation2.ItemIndex > -1) then
    FQualifierItem.Instrument2.TypeOperation := TTypeOperation(cbTypeOperation2.ItemIndex)
  else
...
procedure TfrmQualifierConditionEdit.cbInstrument2IBValue1Change(Sender: TObject);
begin
  if (cbInstrument2IBValue1.ItemIndex > -1) then
    FQualifierItem.Instrument2.TickType1 := TIABTickType(cbInstrument2IBValue1.ItemIndex)
  else
...
procedure TfrmQualifierConditionEdit.cbInstrument2IBValue2Change(Sender: TObject);
begin
  if (cbInstrument2IBValue2.ItemIndex > -1) then
    FQualifierItem.Instrument2.TickType2 := TIABTickType(cbInstrument2IBValue2.ItemIndex)
  else
...
procedure TfrmQualifierConditionEdit.cbTypeConditionChange(Sender: TObject);
begin
  if Showing then
    pcConditions.ActivePageIndex := cbTypeCondition.ItemIndex;
end;

function TfrmQualifierConditionEdit.CheckData: Boolean;
var
  Msg: string;
begin
  Msg := '';
...
procedure TfrmQualifierConditionEdit.aSaveExecute(Sender: TObject);
begin
  ModalResult := mrOk;
end;

function TfrmQualifierConditionEdit.GetInstance: TObject;
begin
  Result := Self;
end;

procedure TfrmQualifierConditionEdit.OnPriceChange(Sender: TObject; Id: Integer; TickType: TIABTickType; Value: Currency; TimeStamp: TDateTime);
begin
...
function TfrmQualifierConditionEdit.GetCalcValue(aQualifierInstrument: TQualifierInstrument): Double;
var
  Value1: Double;
  Value2: Double;
begin
...
procedure TfrmQualifierConditionEdit.CalculateInstrument1;
begin
  if (FQualifierItem.Instrument1.SokidInfo.ContractId > 0) then
  begin
...
procedure TfrmQualifierConditionEdit.CalculateInstrument2;
begin
  if (FQualifierItem.Instrument2.SokidInfo.ContractId > 0) then
  begin
...
procedure TfrmQualifierConditionEdit.aCalculateInstrument1Execute(Sender: TObject);
begin
  CalculateInstrument1;
end;

procedure TfrmQualifierConditionEdit.aCalculateInstrument2Execute(Sender: TObject);
begin
  CalculateInstrument2;
end;

procedure TfrmQualifierConditionEdit.aCalculateInstrument2Update(Sender: TObject);
begin
  TAction(Sender).Enabled := (FQualifierItem.Instrument2.SokidInfo.ContractId > 0);
end;

procedure TfrmQualifierConditionEdit.aCalculateInstrument1Update(Sender: TObject);
begin
