...
uses
  ...
  {$IFDEF USE_CODE_SITE}CodeSiteLogging, {$ENDIF} DaImages, Vcl.ImgList, System.UITypes, CustomForms, Scanner.StaticList,
  Scanner.Types, MessageDialog, Entity.Sokid, Search.Instruments, Monitor.Types, HtmlLib, DebugWriter, Global.Types,
...
type
  TfrmScannerStaticLists = class(TCustomForm)
    aAddAll: TAction;
    aAddList: TAction;
    aCancel: TAction;
  ...
  public
    class function ShowDocument(aDialogMode: TDialogMode; aKindAppend: TKindAppend; aId: Integer; aScannerMarket: IScannerMarket): TModalResult;
    class procedure AddInstrumentToScannerMain(const ATargetForm: IScannerMarket; aKindAppend: TKindAppend; aStaticListsId: Integer; aDialogMode: TDialogMode);
...
class function TfrmScannerStaticLists.ShowDocument(aDialogMode: TDialogMode; aKindAppend: TKindAppend; aId: Integer; aScannerMarket: IScannerMarket): TModalResult;
var
  frmScannerStaticLists: TfrmScannerStaticLists;
begin
  frmScannerStaticLists := TfrmScannerStaticLists.Create(nil);
  try
    frmScannerStaticLists.DialogMode := aDialogMode;
    frmScannerStaticLists.KindAppend := aKindAppend;
    frmScannerStaticLists.Initialize;
    frmScannerStaticLists.fbqStaticLists.Locate('ID', aId, []);
    frmScannerStaticLists.FScannerMarket := aScannerMarket;
    Result := frmScannerStaticLists.ShowModal;
  finally
    frmScannerStaticLists.Denitialize;
    FreeAndNil(frmScannerStaticLists);
  end;
end;

procedure TfrmScannerStaticLists.Initialize;
begin
  DMod.CheckConnect;
  fbqStaticLists.Open;
...
procedure TfrmScannerStaticLists.Denitialize;
begin
  fbqStaticLists.Close;
end;

procedure TfrmScannerStaticLists.grdStaticListsDragDrop(Sender, Source: TObject; X, Y: Integer);
begin
  if (Source is TVirtualStringTree) then
    AddInstruments(TVirtualStringTree(Source));
end;

procedure TfrmScannerStaticLists.AddInstruments(aSourceTree: TVirtualStringTree);
var
  Data: PSokidInfo;
  arrNodes: TNodeArray;
  Node: PVirtualNode;
...
procedure TfrmScannerStaticLists.SaveDetailList(const aParentRecordID: Integer; aNodes: TNodeArray);
resourcestring
...
procedure TfrmScannerStaticLists.grdStaticListsDragOver(Sender, Source: TObject; X, Y: Integer; State: TDragState; var Accept: Boolean);
begin
  Accept := False;
  if (Source is TVirtualStringTree) then
...
procedure TfrmScannerStaticLists.aAddListExecute(Sender: TObject);
begin
  if (TfrmScannerStaticList.ShowDocument(-1) = mrOk) then
    DMod.RefreshQuery(fbqStaticLists);
end;

procedure TfrmScannerStaticLists.aEditListExecute(Sender: TObject);
var
  RecordId: Integer;
begin
  ...
  begin
    RecordId := fbqStaticLists.FieldByName('ID').AsInteger;
    if (TfrmScannerStaticList.ShowDocument(RecordId) = mrOk) then
      DMod.RefreshQuery(fbqStaticLists);
  end;
end;

procedure TfrmScannerStaticLists.aReduceExecute(Sender: TObject);
begin
  if Assigned(FScannerMarket) then
    AddInstrumentToScannerMain(FScannerMarket, kaReduce, fbqStaticLists.FieldByName('ID').AsInteger, DialogMode);
end;

procedure TfrmScannerStaticLists.aIntersectionExecute(Sender: TObject);
begin
  if Assigned(FScannerMarket) then
    AddInstrumentToScannerMain(FScannerMarket, kaIntersection, fbqStaticLists.FieldByName('ID').AsInteger, DialogMode);
end;

procedure TfrmScannerStaticLists.aAddAllExecute(Sender: TObject);
begin
  if Assigned(FScannerMarket) then
    AddInstrumentToScannerMain(FScannerMarket, kaAddAll, fbqStaticLists.FieldByName('ID').AsInteger, DialogMode);
end;

procedure TfrmScannerStaticLists.aUpdateExecute(Sender: TObject);
begin
  if Assigned(FScannerMarket) then
    AddInstrumentToScannerMain(FScannerMarket, kaUpdate, fbqStaticLists.FieldByName('ID').AsInteger, DialogMode);
end;

procedure TfrmScannerStaticLists.aShowSearchInstrumentsExecute(Sender: TObject);
begin
  TfrmSearchInstruments.ShowDocument(Self);
end;

class procedure TfrmScannerStaticLists.AddInstrumentToScannerMain(const ATargetForm: IScannerMarket; aKindAppend: TKindAppend; aStaticListsId: Integer; aDialogMode: TDialogMode);
resourcestring
...
begin
  ...
  begin
    ...
    try
      ...
      if Assigned(ATargetForm) and Supports(ATargetForm, IScannerMarket, ScannerMarket) then
        ScannerMarket.AddInstrument(@Instruments, ColumnsInfo)
      else
        raise Exception.Create('TargetForm not supports IScannerMarket');
    finally
      FreeAndNil(Query);
    end;
  end;
...
procedure TfrmScannerStaticLists.aAddAllUpdate(Sender: TObject);
begin
  TAction(Sender).Enabled := fbqStaticLists.Active and (fbqStaticLists.RecordCount > 0);
end;

procedure TfrmScannerStaticLists.aEditListUpdate(Sender: TObject);
begin
  TAction(Sender).Enabled := fbqStaticLists.Active and (fbqStaticLists.RecordCount > 0) and (DialogMode <> dmView);
end;

procedure TfrmScannerStaticLists.aAddListUpdate(Sender: TObject);
begin
  TAction(Sender).Enabled := (DialogMode <> dmView);
end;

procedure TfrmScannerStaticLists.aDeleteListExecute(Sender: TObject);
resourcestring
...
procedure TfrmScannerStaticLists.aCancelExecute(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TfrmScannerStaticLists.aOkExecute(Sender: TObject);
begin
  ModalResult := mrOk;
end;
