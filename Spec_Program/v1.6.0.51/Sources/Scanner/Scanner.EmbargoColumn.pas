...
uses
  ...
  IABSocketAPI, IABSocketAPI_const, Scanner.Types, DaImages, Utils, Vcl.NumberBox;
{$ENDREGION}

type
  TfrmScannerEmbargoColumn = class(TCustomForm)
...
class function TfrmScannerEmbargoColumn.ShowDocument(aDialogMode: TDialogMode; AColumns: TObjectDictionary<Integer, TColumnsInfo>; var ColumnsInfo: TColumnsInfo): TModalResult;
begin
  Result := mrCancel;
  with TfrmScannerEmbargoColumn.Create(nil) do
  try
...
procedure TfrmScannerEmbargoColumn.Initialize;
var
...
begin
  ...
  if Assigned(FColumns) then
    for Pair in FColumns do
      if Pair.Value.SourceType in [stStaticList, stScannerMarket, stTickColumn, stCalcColumn] then
      begin
...
procedure TfrmScannerEmbargoColumn.Denitialize;
var
...
procedure TfrmScannerEmbargoColumn.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  CanClose := True;
...
function TfrmScannerEmbargoColumn.CheckData: Boolean;
resourcestring
...
procedure TfrmScannerEmbargoColumn.OnChangeEnabled(Sender: TObject);
begin
...
procedure TfrmScannerEmbargoColumn.OnChangePanelColour(Sender: TObject);
begin
...
procedure TfrmScannerEmbargoColumn.cbColumnValueChange(Sender: TObject);
begin
  pnlColumnValueBetween.Visible := cbColumnValue.ItemIndex = Ord(TInequalityType.iqBetween);
end;

procedure TfrmScannerEmbargoColumn.cbRankingPositionChange(Sender: TObject);
begin
  pnlRankingPositionBetween.Visible := cbRankingPosition.ItemIndex = Ord(TInequalityType.iqBetween);
end;

procedure TfrmScannerEmbargoColumn.cbRankingSumChange(Sender: TObject);
begin
