...
begin
  ...
  begin
    ...
    Factor        := nil;
    OrderGroupSet := nil;
    QualifierCondition := nil;

    case Data^.DocType of
      ntQualifier:
        Qualifier := Data^.Qualifier;
      ntQualifierCondition:
        if Assigned(Data^.QualifierCondition) then
          QualifierCondition := Data^.QualifierCondition;
      ntAutoTrade:
      ...
      case Data^.DocType of
        ntQualifier:
          sb.Append(THtmlLib.GetTableLineTag(VarArrayOf(['Name', Qualifier.Name]))).AppendLine
            ...
            .Append(THtmlLib.GetTableLineTag(VarArrayOf(['Description', Qualifier.Description]))).AppendLine;
        ntQualifierCondition:
          sb.Append(THtmlLib.GetTableLineTag(VarArrayOf(['Name', QualifierCondition.Name]))).AppendLine
            .Append(THtmlLib.GetTableLineTag(VarArrayOf(['QualifierId', QualifierCondition.Qualifier.RecordId.ToString]))).AppendLine
            .Append(THtmlLib.GetTableLineTag(VarArrayOf(['TypeCondition', QualifierCondition.TypeCondition.ToString]))).AppendLine
            .Append(THtmlLib.GetTableLineTag(VarArrayOf(['Condition', QualifierCondition.InequalityType.ToString]))).AppendLine
            .Append(THtmlLib.GetTableLineTag(VarArrayOf(['AutoTradeInfo', GetAutoTradeIDs(QualifierCondition.AutoTrades)]))).AppendLine
            .Append(THtmlLib.GetTableLineTag(VarArrayOf(['Bypass', BoolToStr(QualifierCondition.Bypass, True)]))).AppendLine
            .Append(THtmlLib.GetTableLineTag(VarArrayOf(['IsCondition', BoolToStr(QualifierCondition.IsCondition, True)]))).AppendLine
            .Append(THtmlLib.GetTableLineTag(VarArrayOf(['Instrument1', QualifierCondition.Instrument1.SokidInfo.Symbol]))).AppendLine
            .Append(THtmlLib.GetTableLineTag(VarArrayOf(['Instrument2', QualifierCondition.Instrument2.SokidInfo.Symbol]))).AppendLine
            .Append(THtmlLib.GetTableLineTag(VarArrayOf(['StartupDate', FormatDateTime('yyyy.mm.dd hh:nn:ss',QualifierCondition.StartupDate)]))).AppendLine;
        ntAutoTrade:
